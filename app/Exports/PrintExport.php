...
class PrintExport implements FromCollection, WithHeadings
{
    ...
    public function collection()
    {
        ...
        $Prints = Order_Print::where('created_at', '>=', $this->start)->where('created_at', '<=', $this->end)->get();
        foreach ($Prints as $Print) {
            ...
            $order_print->time_discarded_at = $Print->time_discarded_at;
            $order_print->user_skip = $Print->user_skip->name;
            $order_print->user_discarded = $Print->user_discarded->name;
            if ($Print->kind_pay == 0) {
                $order_print->kind_pay = 'chas';
            } else {
                $order_print->kind_pay = 'visa';
            }
            if ($Print->chasier_status == 0) {
                $order_print->chasier_status = 'in chasier';
            } else if ($Print->chasier_status == 1){
                $order_print->chasier_status = 'done';
            }
            ...
            $order_print->discarded = $Print->discarded;
        ...
        return Print_Export::all(['id', 'order','customer_name','customer_phone', 'status', 'machine', 'user_create', 'user_skip', 'user_start', 'time_start_at',
            ...
            'total_cost', 'discount', 'paid', 'rest', 'kind_pay', 'number_visa', 'created_at','discarded','user_discarded','time_discarded_at','chasier_status']);
    }
