...
<div class="wrapper">
    ...
    <div class="content-wrapper">
        ...
        <div class="col-md-12">
            <br>
            <div align="center"><h3>{{ __('Edit Customer') }}</h3></div>
            <div align="right">
            <a href="{{  url('/admin/order/start') }}"  class="btn btn-sm btn-primary">search
                customer</a>
            </div>
            <form autocomplete="off" id="edit" action="{{url('/admin/order/customer/edit/'.$customer->id)}}" method="POST">
                {{csrf_field()}}
                {{method_field('patch')}}
                ...
                <div class="form-group{{ $errors->has('phone') ? ' has-error' : "" }}">
                    phone : <input type="text" value="{{$customer->phone}}" class="form-control" name="phone" placeholder="Enter You phone">
                </div>
